...
class VideoDataSet(torch.utils.data.Dataset):
    def __init__(self, data_type) -> None:
        ...
        self.resnet = create_feature_extractor(self.resnet, {'avgpool':'feat'})
        self.resnet.eval()
    ...
    def __getitem__(self, idx):
        if idx not in self.cached:
            ...
            with torch.no_grad():
                ...
                frame = transforms(frame[::10])
                frame = frame[:20,:]
                frame = frame.float()    
                output_features = self.resnet(frame)
                output_features = output_features['feat'].to('cpu')
